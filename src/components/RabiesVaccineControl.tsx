...
const RabiesVaccineControl: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/.netlify/functions/api-rabies-stats-fallback').then(r => r.json()).then(setStats).catch(() => setStats({ total: 0, caes: 0, gatos: 0, dosesPerdidas: 0, locais: { centro: 0, clinica: 0, hospital: 0 } }));
  }, []);
